...
impl Contract {
    ...
    pub fn internal_swap(
      ...
      //TODO Assert user is authorized to engage with selected vault
    ...
    pub fn add_liquidity(&mut self, pool_id: u64, amounts: Vec<U128>) {
        ...
        for i in 0..tokens.len() {
          // TODO update to allow for insertion of any 1 or more vault tokens
            let amount = *deposits
                .get(&tokens[i])
                .expect(&format!("ERR_MISSING_TOKEN:{}", tokens[i]));
            assert!(
                amounts[i] <= amount,
                "ERR_NOT_ENOUGH_TOKEN:{}", tokens[i]
            );
            if amounts[i] == amount {
